...
@click.option('-d', '--direction', 'low_to_high', type=click.Choice({'DOWN': True, 'UP': False}),
              default='DOWN',
              help="[Default: DOWN] Either start with the lowest amount of colors and go up to max-colors (DOWN), or"
                   "the other way around (UP). See README for more info.")
@click.option('-D', '--dither', required=False, type=click.Choice({'NONE': NONE, 'FLOYDSTEINBERG': FLOYDSTEINBERG}),
              default='NONE',
              help="[Default: NONE] Which dither to use.")
@click.option('-l', '--color-limit-per-tile', required=False, type=int, default=16,
              help="[Default: 16] Limit the tiles to a specific amount of colors they should use before starting. "
...
@click.option('-t', '--transparent-color', type=str, required=False,
              help="A single color value (CSS style, eg. #123456) that should be treated as transparent, when doing"
...
def main(input_image, output_image, loglevel, transparent_color, tile_width, tile_height, **kwargs):
    """
    ...
    with open(input_image, 'rb') as input_file:
        ...
        try:
            converter = ImageConverter(image, tile_width, tile_height, transparent_color)
        except ValueError as err:
            raise ClickException("The image could not be converted: " + str(err))
        img = converter.convert(**kwargs)
